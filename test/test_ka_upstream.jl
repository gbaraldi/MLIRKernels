...
#
# Self-guarded: skips (with @test true) when CUDA isn't functional. We do NOT
# call Testsuite.testsuite(...) because it fans out into backend-protocol
# methods we deliberately don't implement (get_backend(::CuArray), device!,
# partition/mkcontext) and kernels we don't lower (@index Cartesian, @print,
# non-multiple ndrange tail masking); instead we copy the directly-runnable
# kernels, exactly as KA's own backend testsuites are structured.
@testset "KA upstream testsuite kernels (MLIRCUDABackend)" begin
    gpu_ok = try
        @eval using CUDA, LLVM, KernelAbstractions
        @eval using KernelAbstractions.Extras: @unroll
        CUDA.functional()
    catch
        false
    end
    if !gpu_ok
        ...
        b = Base.get_extension(MLIRKernels, :MLIRCUDAExt).MLIRCUDABackend()
        AT = CUDA.CuArray
        @eval begin
        ...
        (@eval _ka_index_linear_global)(b, 8)(A; ndrange=length(A))
        ...
        A = AT{Int}(undef, 64)
        (@eval _ka_localmem)(b, 16)(A; ndrange=size(A))
        KernelAbstractions.synchronize(b)
        Bv = Array(A)
        ...
        A = AT{Int}(undef, 64)
        (@eval _ka_private)(b, 16)(A; ndrange=size(A))
        KernelAbstractions.synchronize(b)
        ...
        (@eval _ka_kernel_unroll!)(b, 1, 1)(a)
        KernelAbstractions.synchronize(b)
